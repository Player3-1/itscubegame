...
const [showMobilePrompt, setShowMobilePrompt] = useState(false);
  ...
  // FIX 2: useEffect içinde 'db' parametresi yerine 'firestoreDb' alias'ı kullanarak
  // firebase db'yi gölgeleme (shadow variable) sorununu ortadan kaldırıyoruz
  useEffect(() => {
  ...
  const saveUserFull = async (updatedUser: User) => {
    ...
    // 4. Mirror to Firestore - firebase 'db' import'unu kullan, lokal değişkeni değil
    try {
      const userRef = doc(db, 'users', updatedUser.name);
      await setDoc(userRef, updatedUser);
      console.log('User saved to Firestore:', updatedUser.name);
    } catch (err) {
  ...
  // FIX 4: Auto-login — Firestore'dan güncel kullanıcı verisi çekiliyor
  useEffect(() => {
    const restoreSession = async () => {
      const sessionUser = localStorage.getItem('nd_user');
      if (!sessionUser) return;

      const parsedSession: User = JSON.parse(sessionUser);

      // Önce Firestore'dan taze veriyi çekmeye çalış
      try {
        const docRef = doc(db, 'users', parsedSession.name);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          const freshUser = docSnap.data() as User;
          // Local cache güncelle
          const usersDb = getUsersDB();
          const idx = usersDb.findIndex(u => u.name === freshUser.name);
          if (idx !== -1) usersDb[idx] = freshUser;
          else usersDb.push(freshUser);
          localStorage.setItem('nd_users_db', JSON.stringify(usersDb));
        ...
        console.error('Auto-login Firestore fetch error:', err);
      }

      // Firestore başarısız olursa local cache'e düş
      const localDb = getUsersDB();
      const freshUser = localDb.find(u => u.name === parsedSession.name);
      if (freshUser) {
        setUser(freshUser);
        loadDraftsForUser(freshUser.name);
      } else {
  ...
  const handleAuthAction = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const usersDb = getUsersDB();
        const idx = usersDb.findIndex(u => u.name === name);
        if (idx !== -1) {
          usersDb[idx] = existingUser;
        } else {
    ...
    if (!existingUser) {
      ...
      existingUser = dbLocal.find(u => u.name === name) || null;
    }
    ...
    if (isRegistering) {
      ...
      await saveUserFull(newUser);
      loadDraftsForUser(newUser.name);
      setGameState(GameState.MENU);
    } else {
      if (!existingUser) {
      ...
      await saveUserFull(existingUser);
      loadDraftsForUser(existingUser.name);
      setGameState(GameState.MENU);
    }
  };
  ...
  const handleAdminCodeSubmit = async () => {
    if (adminCode === ADMIN_PASSWORD) {
      const dbLocal = getUsersDB();
      const adminUser = dbLocal.find(u => u.name === 'dgoa');
      if (adminUser) {
        await saveUserFull(adminUser);
        loadDraftsForUser(adminUser.name);
        setGameState(GameState.MENU);
      }
    } else {
  ...
  const [leaderboardDb, setLeaderboardDb] = useState<User[]>([]);
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        setLeaderboardDb(users.sort((a, b) => (b.totalStars || 0) - (a.totalStars || 0)));
      } catch (err) {
        console.error('Error fetching leaderboard:', err);
        const local = getUsersDB();
        setLeaderboardDb(local.sort((a, b) => (b.totalStars || 0) - (a.totalStars || 0)));
      }
    };
    fetchLeaderboard();
  }, [user, gameState]);
          ...
          <div className="bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-700">
            <div className="flex flex-col items-center gap-4 sm:gap-6">
              ...
              <div className="w-full">
                <h3 className="text-sm uppercase tracking-wide text-slate-400 mb-2">Faces</h3>
              ...
              <div className="w-full">
                <h3 className="text-sm uppercase tracking-wide text-slate-400 mb-2">Colors</h3>
                <div className="flex flex-wrap gap-2 sm:gap-3 justify-center">
          ...
          {openDeals.map((deal) => (
            <div key={deal.id} className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-amber-700 flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-400">by {deal.author}</p>
              </div>
              <div className="flex gap-2">
                <button
